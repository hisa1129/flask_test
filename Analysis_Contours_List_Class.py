...
#        print('first_remove was done')
    ...
    def get_main_angle_list(self, tracking_Results):
        angles_list = []
        eval_tracking_list = [elem for elem in tracking_Results.results if not (math.isnan(elem.main_X))]
        for result_elem in eval_tracking_list:
            if result_elem.main_Y - self.modified_freq_Y != 0:
                slope_XoverY = (result_elem.main_X - self.modified_freq_X)/(result_elem.main_Y - self.modified_freq_Y)
                retValue = math.degrees(math.atan(slope_XoverY))      
            else:
                retValue = 90.0
            angles_list.append(retValue)
        return angles_list
    
    def get_satellite_angle_list(self, tracking_Results):
        angles_list = []
        eval_tracking_list = [elem for elem in tracking_Results.results if not (math.isnan(elem.satellite_X))]
        for result_elem in eval_tracking_list:
            if result_elem.satellite_X - self.modified_freq_Y != 0:
                slope_XoverY = (result_elem.satellite_X - self.modified_freq_X)/(result_elem.satellite_Y - self.modified_freq_Y)
                retValue = math.degrees(math.atan(slope_XoverY))        
            else:
                retValue = 90.0
            angles_list.append(retValue)
