...
def auto_Tracking(
    ...
    if draw_Tracking_Results:
        #追尾結果描画関数の呼び出し
        drawTrackingResult(dirPath=dirPath, trackingRsts=tracking_Results, draw_Fitting_Line = draw_Fitting_Line)        
...
def drawTrackingResult(dirPath, trackingRsts, draw_Fitting_Line):
    '''
    ...
    for i in range(len(pathes)):
        ...
        if delay in [de.delay for de in trackingRsts.results if not math.isnan(de.main_X)]:
            x = [rst for rst in trackingRsts.results if rst.delay == delay][0].main_X
            y = [rst for rst in trackingRsts.results if rst.delay == delay][0].main_Y
            im = cv2.rectangle(im,((int)(x-signature_size),(int)(y-signature_size)),((int)(x+signature_size),(int)(y+signature_size)),\
                               (0,0,255),3)
        if delay in [de.delay for de in trackingRsts.results if not math.isnan(de.satellite_X)]:
            x = [rst for rst in trackingRsts.results if rst.delay == delay][0].satellite_X
            y = [rst for rst in trackingRsts.results if rst.delay == delay][0].satellite_Y
            im = cv2.rectangle(im,((int)(x-signature_size),(int)(y-signature_size)),((int)(x+signature_size),(int)(y+signature_size)),\
                               (255,0,0),3)
